...
module TapeBuilder
  ...
  TAPE_HEIGHT_OFFSET = 500.0  # 胶带上浮高度（毫米）- 悬浮到区域上方
  TAPE_THICKNESS = 100.0  # 胶带厚度（毫米）
  ...
  def self.generate_zone_boundary_tapes(zones_data, parent_group)
    ...
    zones_data.each_with_index do |zone_data, zone_index|
      begin
        ...
        # 使用新的胶带生成算法
        if generate_tape_by_offset_subtraction(points, zone_name, parent_group, tape_layer)
          success_count += 1
        end
        
      rescue => e
        puts "【胶带生成】生成区域 #{zone_data["name"] || zone_data["id"]} 胶带失败: #{Utils.ensure_utf8(e.message)}"
      end
    end
    
    puts "【胶带生成】完成！成功生成 #{success_count}/#{total_count} 个区域的胶带"
  end
  ...
  def self.generate_outdoor_zone_boundary_tapes(zones_data, parent_group)
    ...
    zones_data.each_with_index do |zone_data, zone_index|
      begin
        ...
        if generate_tape_by_offset_subtraction(points, zone_name, parent_group, tape_layer, true)
          success_count += 1
        end
        
      rescue => e
        puts "【胶带生成】生成外部区域 #{zone_data["name"] || zone_data["id"]} 胶带失败: #{Utils.ensure_utf8(e.message)}"
      end
    end
    
    puts "【胶带生成】外部区域边界胶带生成完成！成功生成 #{success_count}/#{total_count} 个外部区域的胶带"
  end
  ...
  def self.generate_tape_by_offset_subtraction(points, zone_name, parent_group, tape_layer, is_outdoor = false)
    ...
    # 生成大区域（向外偏移半个宽度）
    large_region_points = generate_offset_polygon(points, half_width, "外扩")
    return false unless large_region_points
    
    # 生成小区域（向内偏移半个宽度）
    small_region_points = generate_offset_polygon(points, -half_width, "内缩")
    return false unless small_region_points
    
    # 生成胶带：大区域减去小区域
    tape_polygon = subtract_polygons(large_region_points, small_region_points, zone_name)
    return false unless tape_polygon
  ...
  # 生成偏移多边形
  def self.generate_offset_polygon(original_points, offset_distance, direction_name)
    ...
    # 计算偏移线段的交点，形成闭合多边形
    offset_polygon = calculate_offset_polygon_intersections(offset_points, direction_name)
  ...
  # 计算偏移多边形的交点
  def self.calculate_offset_polygon_intersections(offset_lines, direction_name)
    ...
    if offset_lines.size > 2
      ...
      if closing_intersection
        ...
        puts "【胶带生成】#{direction_name}: 成功闭合多边形"
      end
    end
