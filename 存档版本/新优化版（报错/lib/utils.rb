module Utils
  ...
  # 检查两个数组是否接近（用于流程排序）
  def self.arr_close?(a, b, tol=1.0)
    a && b && a.size>=2 && b.size>=2 && (a[0]-b[0]).abs<tol && (a[1]-b[1]).abs<tol
  end
  ...
  # 检查两个点是否相同（考虑精度误差）
  def self.points_equal?(p1, p2, tolerance = 1.0)
    return false unless p1 && p2
    (p1.x - p2.x).abs < tolerance && (p1.y - p2.y).abs < tolerance && (p1.z - p2.z).abs < tolerance
  end
  
  # 检查两个线段是否共享边界
  def self.segments_share_boundary?(seg1_start, seg1_end, seg2_start, seg2_end, tolerance = 1.0)
  ...
  # 检查点是否在线段上（忽略Z轴）
  def self.point_on_segment_2d?(point, seg_start, seg_end, tolerance = 1.0)
  ...
  # 为紧邻区域添加微小偏移以避免面冲突
  def self.add_zone_offset(points, zone_id, offset_distance = 1.0)
    return points if points.size < 3
