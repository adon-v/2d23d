  ...
  def self.import_walls(walls_data, parent_group)
    ...
    walls_data.each do |wall_data|
      begin
        ...
        thickness = Utils.parse_number(wall_data["thickness"] || 200.0)
        puts "t"
        puts thickness
        height = Utils.parse_number(wall_data["height"] || 3000.0)
        
        thickness = 200.0 if thickness < 0 || thickness.nil?
        height = 3000.0 if height <= 0 || height.nil?
        ...
        if thickness > 0 && !outwall_edges.empty?
          outwall_edges.each do |ow_s, ow_e|
            # 判断两线段是否共线且端点重合/极近
            if (start_point.distance(ow_s) < 1.0 && end_point.distance(ow_e) < 1.0) ||
               (start_point.distance(ow_e) < 1.0 && end_point.distance(ow_s) < 1.0)
              skip = true
              break
            end
          end
    ...
    begin
      ...
      all_edges.combination(2).each do |e1, e2|
        ...
        if (pts1[0].distance(pts2[0]) < 1.0 && pts1[1].distance(pts2[1]) < 1.0) ||
           (pts1[0].distance(pts2[1]) < 1.0 && pts1[1].distance(pts2[0]) < 1.0)
          
          v1 = pts1[1] - pts1[0]; v2 = pts2[1] - pts2[0]
          
          if v1.length > 1.0 && v2.length > 1.0 && (v1.normalize.dot(v2.normalize).abs > 0.999)
            [e1, e2].each do |e|
              next if processed[e]
